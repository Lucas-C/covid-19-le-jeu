...
function sickedPawns(board) {
  return wrapAnimDelay(() => board.allPlanets.forEach((planet) => { // pour chaque planète
    ...
    const incubating = planet.getAllPawnsWithState('incubating');// je récupère les pions incubés
    ...
    const incubating = element.getAllPawnsWithState('incubating');
    if (incubating !== null) { // s'il y en a
    ...
    const incubating = board.robotAcademy.getAllPawnsWithState('incubating');
    if (incubating !== null) { // s'il y en a
    ...
    .then(wrapAnimDelay(() => {
      let incubating = board.batterieMarketZ1.getAllPawnsWithState('incubating');// je récupère les pions incubés
      if (incubating !== null) { // s'il y en a
        incubating.forEach((pawn) => {
          pawn.setState('sick'); // je les passe malade
        });
...
function infectPawns(board) {
  ...
  return wrapAnimDelay(() => board.allPlanets.forEach((planet) => { // pour chaque planète
    if (planet.isContaminated()) {
      ...
      let sanes = planet.getAllPawnsWithState('sane');
      if (sanes === null) {
        sanes = [];
      }
      const toIncubate = Math.min(sanes.length, (planet.coefInfection * planet.getAllPawnsWithState('sick').length) - board.bonusInfection);
      console.debug('Nb à infecter :', toIncubate);
      messageDesc(board, '[Étape 2] Nombre de robots infectés : ', toIncubate);
      for (let i = 0; i < toIncubate; i++) {
    ...
    if (element.isContaminated()) {
      ...
      let sanes = element.getAllPawnsWithState('sane');
      if (sanes === null) {
        sanes = [];
      }
      const toIncubate = Math.min(sanes.length, (element.coefInfection * element.getAllPawnsWithState('sick').length) - board.bonusInfection);
      messageDesc(board, '[Étape 2] Nombre de robots infectés : ', toIncubate);
      for (let i = 0; i < toIncubate; i++) {
        sanes[i].setState('incubating');
      }
    ...
    if (board.robotAcademy.isContaminated()) {
      ...
      let sanes = board.robotAcademy.getAllPawnsWithState('sane');
      if (sanes === null) {
        sanes = [];
      }
      const toIncubate = Math.min(sanes.length, (board.robotAcademy.coefInfection * board.robotAcademy.getAllPawnsWithState('sick').length) - board.bonusInfection);
      messageDesc(board, '[Étape 2] Nombre de robots infectés : ', toIncubate);
      for (let i = 0; i < toIncubate; i++) {
        sanes[i].setState('incubating');
      }
    ...
    if (board.batterieMarketZ1.isContaminated()) {
      ...
      let sanes = board.batterieMarketZ1.getAllPawnsWithState('sane');
      if (sanes === null) {
        sanes = [];
      }
      const toIncubate = Math.min(sanes.length, (board.batterieMarketZ1.coefInfection * board.batterieMarketZ1.getAllPawnsWithState('sick').length) - board.bonusInfection);
      messageDesc(board, '[Étape 2] Nombre de robots infectés : ', toIncubate);
      for (let i = 0; i < toIncubate; i++) {
        sanes[i].setState('incubating');
      }
    ...
    if (board.batterieMarketZ2.isContaminated()) {
      ...
      let sanes = board.batterieMarketZ2.getAllPawnsWithState('sane');
      if (sanes === null) {
        sanes = [];
      }
      const toIncubate = Math.min(sanes.length, (board.batterieMarketZ2.coefInfection * board.batterieMarketZ2.getAllPawnsWithState('sick').length) - board.bonusInfection);
      messageDesc(board, '[Étape 2] Nombre de robots infectés : ', toIncubate);
      for (let i = 0; i < toIncubate; i++) {
        sanes[i].setState('incubating');
      }
