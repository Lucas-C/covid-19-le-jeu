...
function sickedPawns(board) {
  return wrapAnimDelay(() => board.allPlanets.forEach(planet => { // pour chaque planète
    ...
    if(incubating !== null){// s'il y en a
  ...
  })).then(messageDesc(board, 'Les robots infectés deviennent malades dans les planètes maisons')).then(messageDesc(board, 'Les robots infectés deviennent malades dans les lieux publics')).then(wrapAnimDelay( () => board.allPublicPlaces.forEach(element => { // pour chaque lieu public
      ...
      if(incubating !== null){// s'il y en a
        messageDesc(board, 'Les robots infectés deviennent malades à la robot académie')
        incubating.forEach(pawn => { // je récupère les pions incubés
      ...
      messageDesc(board, 'Les robots infectés deviennent malades au supermarché');
...
function infectPawns(board) {
  ...
  return wrapAnimDelay( () => board.allPlanets.forEach(planet => { // pour chaque planète
    if (planet.isContaminated()) {
      ...
      messageDesc(board, `La planète ${planet.name} est contaminée`);
      ...
      messageDesc(board, 'Nombre de robots infectés : ', toIncubate);
      ...
      if (element.isContaminated()) {
        messageDesc(board, `Le lieu public ${element.name} est contaminé`);
       ...
       if (board.robotAcademy.isContaminated()) {
        messageDesc(board, 'La robot académie est contaminée');
      ...
      if (board.batterieMarketZ1.isContaminated()) {
       messageDesc(board, 'La zone 1 du batterieMarket est contaminée');
      ...
      if (board.batterieMarketZ2.isContaminated()) {
        messageDesc(board, 'La zone 2 du batterieMarket est contaminée');
